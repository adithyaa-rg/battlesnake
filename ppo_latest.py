...
def minimax_policy(observation: np.ndarray, action_space: gym.spaces.Discrete) -> int:
    """
    An improved minimax-based policy for the Battlesnake agent with depth-2 search.
    
    Args:
        observation (np.ndarray): 4-channel grid (self, teammate, enemies, food).
        action_space (gym.spaces.Discrete): Action space (0: up, 1: down, 2: left, 3: right).
    ...
    # Find agent's head (assumed marked with 5)
    agent_layer = observation[0]
    head_pos = np.where(agent_layer == 5)
    if len(head_pos[0]) == 0:
        # Fallback if head not found
        agent_parts = np.argwhere(agent_layer > 0)
        if len(agent_parts) == 0:
            return action_space.sample()
        head_r, head_c = agent_parts[0]
    else:
        head_r, head_c = head_pos[0][0], head_pos[1][0]
    
    # Define moves: (delta_row, delta_col, action_index)
    ...
    def is_valid_position(r, c):
        ...
        return (0 <= r < H and 0 <= c < W and 
                observation[0, r, c] == 0 and 
                observation[1, r, c] == 0 and 
                observation[2, r, c] == 0)
    
    def get_free_adjacent(r, c):
        """Count free adjacent cells."""
        free_count = 0
        for dr, dc, _ in moves:
            nr, nc = r + dr, c + dc
            if 0 <= nr < H and 0 <= nc < W:
                if observation[0, nr, nc] == 0 and observation[1, nr, nc] == 0 and observation[2, nr, nc] == 0:
                    free_count += 1
        return free_count
    
    def distance_to_nearest(position, positions):
        """Compute Manhattan distance to nearest position in array."""
        if len(positions) == 0:
            return 10 * (H + W)  # Large value if no positions
        min_dist = float('inf')
        r, c = position
    ...
    def heuristic(r, c):
        """Evaluate state with a comprehensive heuristic."""
        if not (0 <= r < H and 0 <= c < W):
            return -float('inf')  # Out of bounds
        if observation[0, r, c] > 0 or observation[1, r, c] > 0 or observation[2, r, c] > 0:
            return -float('inf')  # Collision
        
        food_positions = np.argwhere(observation[3] > 0)
        enemy_positions = np.argwhere(observation[2] > 0)
        pos = (r, c)
        
        on_food = observation[3, r, c] > 0
        food_dist = distance_to_nearest(pos, food_positions)
        enemy_dist = distance_to_nearest(pos, enemy_positions)
        free_adj = get_free_adjacent(r, c)
        
        score = 0
        if on_food:
            score += 100  # High reward for eating food
        score += free_adj * 5  # Value open space
        score -= 10 / (enemy_dist + 1)  # Avoid enemies
        score += 20 / (food_dist + 1)  # Approach food
        return score
    
    def simulate_opponent_response(agent_r, agent_c):
        """Approximate worst-case enemy move toward agent or food."""
        enemy_positions = np.argwhere(observation[2] > 0)
        if len(enemy_positions) == 0:
            return heuristic(agent_r, agent_c)
        
        # For simplicity, consider enemies moving toward agent
        worst_score = float('inf')
        for er, ec in enemy_positions[:1]:  # Limit to one enemy for efficiency
            for dr, dc, _ in moves:
                ner, nec = er + dr, ec + dc
                if not is_valid_position(ner, nec):
                    continue
                # Update observation approximation
                temp_obs = observation.copy()
                temp_obs[2, er, ec] = 0  # Clear old position
                temp_obs[2, ner, nec] = 5  # New enemy head
                score = heuristic(agent_r, agent_c)
                worst_score = min(worst_score, score)
        return worst_score if worst_score != float('inf') else heuristic(agent_r, agent_c)
    
    # Depth-2 minimax evaluation
    ...
    for dr, dc, action in moves:
        ...
        if is_valid_position(nr, nc):
            ...
            # Depth 2: Opponent's response
            score = simulate_opponent_response(nr, nc)
            if score > best_score:
                best_score = score
                best_actions = [action]
    ...
    # Return random choice among best actions or sample if no valid moves
    return np.random.choice(best_actions) if best_actions else action_space.sample()
...
class PPOTrainer:
    def __init__(self,
        ...
        self.existing_policies: List[Any] = [random_policy, initial_random_policy, minimax_policy] # Can hold PolicyValueNetwork or functions
        if self.ckpt_dir:
    ...
    def _update_existing_policies(self):
        ...
        self.existing_policies = [minimax_policy, initial_random_policy, random_policy] # Start with the random policy
    ...
    def _set_policies_for_agents(self) -> Tuple[List[Any], List[int]]:
        ...
        The next two are opponents chosen with 70% minimax, 20% initial random, and 10% random probabilities.
        """
        # Policies for the learning agents (snakes 0 and 1)
        learning_agent_policies = [agent.policy for agent in self.agents[:self.num_learning_agents]]
        
        # Indices to track original policy type (-1 for learning agents)
        policy_source_indices = [-1] * self.num_learning_agents
        ...
        # Define the policies and their probabilities for opponents
        policies = [minimax_policy, initial_random_policy, random_policy]
        probabilities = [0.7, 0.2, 0.1]
        
        # Assign policies to opponent agents (snakes 2 and 3)
        for _ in range(num_opponents):
            # Choose a policy based on the specified probabilities
            chosen_policy = np.random.choice(policies, p=probabilities)
            opponent_policies.append(chosen_policy)
            opponent_source_indices.append(-1)  # Use -1 as a marker since we're not tracking indices

        # Combine policies for all snakes
        all_game_policies = []
        all_game_policy_source_indices = []

        # Learning agents (snakes 0 and 1)
        for i in range(self.num_learning_agents):
            all_game_policies.append(learning_agent_policies[i])
            all_game_policy_source_indices.append(policy_source_indices[i])

        # Opponent agents (snakes 2 and 3)
    ...
    def train(self, total_training_timesteps: int):
        ...
        for t_step_global in range(1, total_training_timesteps + 1):
            ...
            done_by_env1 = terminated_all_snakes_list[0] and terminated_all_snakes_list[1] # If both learning agents are out
            ...
            is_episode_done = done_by_opponents or done_by_maxlen or done_by_env1
            ...
            if is_episode_done:
                ...
                if self.total_episodes_completed % self.log_interval_episodes == 0 and len(self.episode_rewards_buffer) > 0:
                    ...
                    logging.info(
                        ...
                        f"Avg Reward (last {self.log_interval_episodes}): {avg_reward:.2f} | Win %: {(win_count / self.total_episodes_completed * 100):.2f} | Avg Len: {avg_length:.2f} | Time: {elapsed_time:.2f}s"
                    )
...
def main():
    ...
    config: Dict[str, Any] = {
        "env_id": 'BattleSnake',
        "total_training_timesteps": 7_000_000, # This is per-agent effectively, or total env steps
        "rollout_steps": 2048,
        "hidden_dim": 64,
        "lr": 3e-4,
        "gamma": 0.99,
        ...
        "ckpt_dir": "./models/new_battlesnake_ppo_hopefully_final", # Directory to save models
        # Example: "./models/PPOBattlesnake_Corrected/agent_0_steps_50000.pth"
        "load_model_path_agent0": "./models/new_battlesnake_ppo_hopefully_final/agent_1_steps_15150001.pth", # Path for agent 0
        "load_model_path_agent1": "./models/new_battlesnake_ppo_hopefully_final/agent_0_steps_15150001.pth", # Path for agent 1
